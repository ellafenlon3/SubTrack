...
if "confirm_reminder" not in st.session_state:
    st.session_state.confirm_reminder = False
...
# --- Set Reminder Flow ---
elif st.session_state.confirm_reminder:
    st.markdown("""
        <div style='
            background-color: white;
            padding: 40px;
            ...
            margin-top: 60px;
        '>
            <h2 style='color: #333;'>⏰ Set a reminder for this trial?</h2>
        </div>
    """, unsafe_allow_html=True)

    col1, col2 = st.columns([1, 1])
    with col1:
        if st.button("🔙 Go Back (Reminder)"):
            st.session_state.confirm_reminder = False

    with col2:
        if st.button("✅ Yes, Set Reminder"):
            st.success("Reminder has been set successfully.")
            st.session_state.confirm_reminder = False
...
# --- Main View ---
    ...
    with col1:
        if st.button("Set Reminder"):
            st.session_state.confirm_reminder = True
    with col2:
        if st.button("Cancel Subscription"):
            st.session_state.confirm_cancel = True
